...
WaveSurfer.Drawer = {
    ...
    recenterOnPosition: function (position, immediate) {
        var scrollLeft = this.container.scrollLeft; //scrollbar已移動長度
        var max = this.container.clientWidth; //scrollbar當前視窗大小
        var target = position - max;
        var offset = target - scrollLeft;

        // if the cursor is currently visible...
        if (!immediate && offset >= -max && offset < max) {
            // we'll limit the "re-center" rate.
            var rate = 5;
            offset = Math.max(-rate, Math.min(rate, offset));
            target = scrollLeft + offset + this.container.clientWidth;
        }
