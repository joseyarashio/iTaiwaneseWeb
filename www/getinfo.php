<?php
	...
	$result=mysqli_query($link,"SELECT `location`, `value` FROM `file` WHERE id=\"".$_GET['id']."\"");
	...
	if($row[0]!=""){
		...
		mysqli_query($link,"INSERT INTO `visitinfo`(`visitor`, `ip`, `target`, `date`) VALUES ('問題-試圖找尋檔案','".$ip."','".$_GET['id']."','".$tdate."')");
		header("Location:editablepage.php");
	}
	mysqli_close($link);
