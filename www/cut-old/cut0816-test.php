...
<body>
	...
	<script type="text/javascript">
		...
		var pre_speakerList = new Array();
		var speakerList = new Array();
		var speakerDev = new Array();
		...
		function splitText(msg,file_name){
			...
			pre_speakerList = msg.split(/<\?.*\?>|<\/Turn>|<Sync time=.*>|tbw.*|ttsup.*|ttsu.*|<.*Section.*>|<.*Episode.*>|<.*Trans.*>|<.*Speaker.*>/);
			...
			while(i != pre_time.length){//處理時間陣列
				...
				if(/<Turn.*/.test(pre_text[i])){	
					speakerDev[k] = m;//spk句數
					k++;
				}
				i++;
			...
			//speakerList
			i = 0; m = 0;
			while(i != pre_speakerList.length){
				if(/<Turn.*/.test(pre_speakerList[i])){
					speakerList[m] = pre_speakerList[i];
					m++
				}	
				i++;
			}
			...
			while(text.length > i){//將時間+文字顯示
				if(k < speakerList.length && i == speakerDev[m]){
					var spk = speakerList[m].split(/<Turn.*speaker="|".*>/);
					$("#S_result").append("<button type='button'>"+spk[1]+"</button>");
					m++;
				}
