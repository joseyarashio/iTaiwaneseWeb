<?php $link = mysqli_connect('localhost', 'user', 'user', 'itaiwanese');
	...
	while ($row = mysqli_fetch_row($base)){
		...
		$file=mysqli_query($link,"SELECT `id`,`name`,`value`,`end`,`accesskey` FROM `file` WHERE `base`='".$basename."'");
		...
		while ($row = mysqli_fetch_row($file)){
			...
			echo"<td><a href=\"getinfo_accesskey.php?id=".$row[0]."&accesskey=".$row[4]."\">".$row[1]."</a></td>";
	...
	if(isset($_COOKIE['Value'])&&$_COOKIE['Value']==10){
		...
		echo"<br><input type=\"button\" onClick=\"location.href='./addbase.php?orig=editablepage.php'\" value='增加新欄位'><input type=\"button\" onClick=\"location.href='edit_basefile.php?orig=editablepage.php'\" value='編輯欄位'><input type=\"button\" onClick=\"location.href='./random.php'\" value='變換accesskey'>";
	}
